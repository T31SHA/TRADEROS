...
class StrategyRuntimeBinding:
    """Explicit mapping from a governed implementation identity to code."""
...
class PaperDecisionPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        self._bindings = {
            (item.implementation_id, item.implementation_version): item
    ...
    def run(
        ...
        *,
        ...
        if now - decision_timestamp > self.paper_engine.config.decision_max_age:
            return self._blocked(
                ("DATA_DECISION_STALE",),
        ...
        if decision_timestamp - quote_timestamp > self.paper_engine.config.quote_max_age:
            return self._blocked(
                ("DATA_QUOTE_STALE",),
        ...
        elif system_health.timestamp > decision_timestamp:
            pending_health_reason = "SYSTEM_HEALTH_FUTURE_DATED"
        elif (
            decision_timestamp - system_health.timestamp
        ...
        if pending_health_reason is None:
            ...
            trace.append(
                PipelineTraceEvent("pending_orders", "HEALTHY", f"FILLS={len(fills)}")
            )
        else:
            trace.append(
                PipelineTraceEvent(
        ...
        for strategy_id, strategy_version in configured_keys:
            ...
            if (
                ...
                and artifact.dataset_identity != dataset_hash
            ):
                resolution_reasons.append(
        ...
        if not strategies:
            return self._blocked(
                ...
                (
                    *trace,
                    PipelineTraceEvent(
                        "strategy_resolution", "BLOCKED", resolution_reasons[0]
